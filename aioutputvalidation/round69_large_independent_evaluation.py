"""Evaluate the completed frozen 90-case Round 69 blinded confirmation."""
...
def evaluate() -> dict:
    packets = [json.loads(path.read_text(encoding="utf-8")) for path in sorted((OUT / "output_review_packets").glob("SYN-*.json"))]
    # The 90 packets were completed with the concise UI status `completed`.
    # Normalize it in memory; do not rewrite human review records.
    packets = [{**packet, "review_status": "completed_human_output_review"} if packet.get("review_status") in {"completed", "human_output_review_complete"} else packet for packet in packets]
    incomplete = [packet["case_id"] for packet in packets if packet.get("review_status") not in {"completed_human_output_review", "reviewed"} or packet.get("reviewer_decision") not in {"safe", "review_required", "high_risk"}]
    if incomplete:
        raise ValueError("incomplete output reviews: " + ", ".join(incomplete))
    if len(packets) != 90:
        raise ValueError(f"expected 90 output-review packets, found {len(packets)}")
    ...
    (results_dir / "summary.json").write_text(json.dumps({"validated_outputs": len(results), "decision_counts": dict(Counter(item["decision"] for item in results)), "model_version": manifest["active_model_version"], "mode": "shadow_evaluation"}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    matches = 0
    decision_matrix = Counter()
    for result in results:
        reviewer_decision = human[result["case_id"]]
        matches += int(result["decision"] == reviewer_decision)
        decision_matrix[f"{result['decision']} -> {reviewer_decision}"] += 1
    output = {
    ...
    for path in sorted((OUT / "contrastive_label_packets").glob("SYN-*.json")):
        ...
        if packet.get("labeling_status") in {"reviewed", "completed", "human_review_complete"}:
    ...
    report = {"round": 69, "scope": "large_blinded_confirmation_30_per_tier", "output_validation": output, "contrastive": contrastive}
    for name, value in (("round69_output_evaluation.json", output), ("round69_contrastive_evaluation.json", contrastive), ("round69_large_independent_evaluation.json", report)):
