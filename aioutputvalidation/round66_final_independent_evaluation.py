"""Evaluate completed Round 66 final independent human reviews."""
...
def binary_metrics(results: list[dict], packets: list[dict]) -> dict:
    ...
    for result in results:
        ...
        actual_unsafe = human[result["case_id"]] != "safe"
...
def evaluate() -> dict:
    ...
    report = {"round": 66, "scope": "final_independent_confirmation", "output_validation": output_report, "contrastive": contrastive_report}
    (OUT / "round66_output_evaluation.json").write_text(json.dumps(output_report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (OUT / "round66_contrastive_evaluation.json").write_text(json.dumps(contrastive_report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (OUT / "round66_final_evaluation.json").write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return report
