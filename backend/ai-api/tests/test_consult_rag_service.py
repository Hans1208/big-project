...
def test_consult_rag_reads_only_anonymized_text():
    ...
    def fake_statutes(
        *,
        ...
        calls["statutes"].append(
            (anonymized_text, top_n)
        )
        return [{"citation": "statute"}]
    ...
    results = collect_related_legal_sources(
        content=content,
        top_n=5,
        statute_search=fake_statutes,
        precedent_search=fake_precedents,
    )

    expected_text = content["anonymized_text"]

    assert calls["statutes"] == [
        (expected_text, 5)
    ]
    assert calls["precedents"] == [
        (expected_text, 5)
    ]
    assert "RAW-SUMMARY-SECRET" not in str(calls)
    assert "RAW-DETAIL-SECRET" not in str(calls)
