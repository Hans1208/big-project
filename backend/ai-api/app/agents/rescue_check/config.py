...
# 이혼/상속/가족관계: 청구권별로 기산일·기간이 크게 다름(예: 양육비는 정기금 채무라
#   회차별로 기산일이 달라지고, 재산분할청구권은 2년, 상속회복청구권은 안 날로부터 3년/
#   있은 날로부터 10년 등) -> 아직 팀 리뷰로 세분화되지 않았으므로 의도적으로 None을 두어
#   "표에 없어서 빠진 것"이 아니라 "리뷰 대기 중이라 계산 불가로 처리"임을 명시한다.
#   (Rule Engine은 None이면 LLM이 무엇을 추출했든 항상 "계산 불가"로 강제한다 -> graph.py 참고)
