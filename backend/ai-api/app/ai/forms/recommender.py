...
#
# helplaw24의 대분류는 '주제'가 아니라 '어디에 내는 서류인가'로 갈리는 곳이 있다.
# 그래서 상담을 정확히 분류해도 정작 필요한 서식이 후보에 못 들어오는 자리가 생긴다.
#
# 상속 -> 가사소송/라,마류 가사비송 (8건)
#   상속 '절차' 서식이 상속이 아니라 가사비송에 있다.
#     상속한정승인 심판청구서 / 상속재산포기 심판청구서 / 상속승인기간 연장허가청구서 등
#   반면 "상속" 대분류는 15건뿐이고 전부 분할·유언·유류분 계열이다.
#
# 친족 -> 가족관계등록/신고 (20건)
#   같은 구조다. 친족 대분류에는 법원에 내는 청구서만 있고 신고서가 0건이다.
#   관공서에 내는 신고서는 전부 가족관계등록/신고에 있다.
#     혼인신고서 / 입양신고서 / 파양신고서 / 후견개시신고서 /
#     이혼(친권자 지정)신고서 / 친권자(지정, 변경)신고서 등
#   협의이혼하며 친권자를 정하는 상담은 친족으로 분류되는데, 정작 낼 신고서가 여기 없다.
#
...
#
# 여기에 넣지 않은 것들:
#   친족 -> 가사소송/가,나,다류 가사소송 (33건: 혼인무효·친생부인·입양취소 등)
#   친족 -> 가사소송/이행명령·양육비직접지급명령 (3건)
#   이들은 분류 체계가 어긋난 게 아니라 '진짜 가사소송'이다. 후보를 넓힐 문제가 아니라
#   상담을 처음부터 가사소송으로 분류해야 하는 문제라, prompts.py의 단계 판단 규칙이 담당한다.
CROSS_CATEGORY_SUBS = {
    "상속": [("가사소송", "라,마류 가사비송")],
    "친족": [("가족관계등록", "신고")],
}
...
def get_candidates(case_type: str, case_subtype: str = None, query_text: str = None) -> list:
    ...
    # 대분류가 어긋나 있는 곳을 항상 함께 넣는다. 결과가 빈 뒤에 넓히는 게 아니라
    # 처음부터 후보에 둔다 — 그럴듯한 오답이 채워지면 확장이 발동하지 않기 때문이다.
