...
// 진행 중인 통화(외부 서버 레그 + 오퍼레이터 레그)를 관리하고 둘 사이에서 오디오 바이트를 중계하는
// 교환대(switchboard). 저장은 메모리다 — AudioStreamTicketService와 같은 이유로 지금은 서버가
// 한 대뿐이라 충분하다.
@Component
public class CallRegistry {
    ...
    void relayToOperator(String callId, ByteBuffer payload) {
        Call call = calls.get(callId);
        if (call != null && call.operatorSession() != null) {
            sendQuietly(call.operatorSession(), payload);
        }
    }
    ...
    private static void sendQuietly(WebSocketSession session, ByteBuffer payload) {
        try {
            if (session.isOpen()) {
                session.sendMessage(new BinaryMessage(payload));
            }
        } catch (IOException e) {
            log.warn("failed to relay audio to session={}", session.getId(), e);
        }
    }
