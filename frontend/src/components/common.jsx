...
function evidenceBadge(row) {
  ...
  if (!check.isTargetCandidate) return { label: '대상 아님', tone: 'neutral', title: '법률구조 대상 유형에 해당하지 않습니다.' };
  if (check.evidenceSubmitted) return { label: '구조대상', tone: 'submitted', title: `법률구조 대상자 증빙 제출 확인: ${check.requiredEvidence}` };
  return { label: '증빙 필요', tone: 'missing', title: `법률구조 대상자 증빙 미제출: ${check.requiredEvidence}` };
}
...
function ConsultationTable({ title, rows, onAdd, onDelete, onOpenAnalysis, tall = false, selectedDate, onDateChange, searchable = false }) {
  ...
  const scrollable = !tall && displayRows.length > visibleRowCount;
      ...
      <div className="panelTitleRow">
        ...
        {searchable ? (
          <div className="tableSearchBox">
            ...
            <input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="이름·사건번호·제목 검색" aria-label="상담 검색" />
          </div>
        ) : null}
          ...
          <tbody>
            ...
            {displayRows.map((row) => (
              <tr key={row.id}>
                ...
                <td>
                  {/* 배지를 먼저 두어 같은 행의 상태·처리단계·삭제 칩과 같은 줄에 오게 하고,
                      실제 등록일시는 그 아래 보조 정보로 한 줄에 붙입니다. */}
                  <div className="cellBody registeredCell">
                    <span className={`dayBadge${dayBadgeLabel(row.date) === 'TODAY' ? ' today' : ''}`}>{dayBadgeLabel(row.date)}</span>
                  </div>
                </td>
                <td>
                  <div className="cellBody">
            ...
            {scrollable || noSearchResult ? null : <EmptyRows count={Math.max(0, visibleRowCount - displayRows.length)} columns={columns} />}
          </tbody>
        </table>
      </div>
    </section>
...
function HitlConfirmModal({ title = 'AI 분석 결과 최종 확인', actionLabel = '작업 진행', caseInfo, onConfirm, onCancel }) {
  return (
